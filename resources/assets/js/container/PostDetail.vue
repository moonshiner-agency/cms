<template>
    ...
    <article>
        ...
        <div class="main">
            ...
            <div class="post">
                <div class="title">
                    <input type="text" v-model="currentPost
.title">
                </div>
            ...
            <template v-if="templateStructure != null">
                <Panel :headline="templateGroup.name" v-for="templateGroup in templateStructure.additional_fields" key="templateGroup.name">
                    <AdditionalField v-bind:content="currentPost.content[templateGroup.name]" v-bind:template="templateGroup" @changed="newContent => { 
                // copy content
        ...
        <div class="sidebar">
            <!-- START Settings Box -->
            <Panel headline="Publish">
                <CmsOption parameter="Author" :status="currentPost.author">
                    <input slot="input" type="text" v-model="currentPost.author">
                </CmsOption>
                <CmsOption parameter="Category" :status="currentPost.category">
                    <select v-model="currentPost.category" slot="input">
                        <option v-for="category in categories" :value="category">{{ category }}</option>
                    </select>
                </CmsOption>
                <CmsOption parameter="Status" :status="currentPost.post_status">
                    <select v-model="currentPost.post_status" slot="input">
                        <option value="published">Published</option>
                        <option value="draft">Draft</option>
                    </select>
                </CmsOption>
                <CmsOption parameter="Visibility" :status="currentPost.visibility">
                    <select v-model="currentPost.visibility" slot="input">
                        <option value="public">Public</option>
                        <option value="private">Private</option>
                    </select>
                </CmsOption>
                <CmsOption parameter="Published on: " :status="currentPost.published_at">
                    <input slot="input" type="text" v-model="currentPost.published_at">
                </CmsOption>
            ...
            <!-- END Template Box -->
            <!-- START Template Box -->
            <Panel headline="Featured Image">
                <File :updatePath="newPath => { currentPost.featured_image = newPath }" :path="currentPost.featured_image" />
            </Panel>
            <!-- END Template Box -->
        </div>
        <Overlay v-show="msg">
...
export default {
    ...
    mounted: function() {
        if (this.post) {
            ...
            fetch('GET', `posts/${this.post.id}`, this.postLoaded);
        } else {
            //fresh Page
            this.postLoaded({
                post: {
    ...
    methods: {
        ...
        updatePost: function() {
            ...
            fetch('PUT', `posts/${this.post.id}`, this.postLoaded, this.currentPost);
        },
        savePost: function() {
            ...
            fetch('POST', `posts/`, this.postLoaded, this.currentPost);
        },
        ...
        deletePost: function() {
            ...
            fetch('DELETE', `posts/` + this.currentPost.id, this.deleteSuccess);
        },
